...
export default class Card {
  constructor(
    ...
    { handleClickLikeCard },
    { handleOpenImgCardClick },
    { popupConfirmCardDelete }
  ) {
    this._name = dataCard.name; // имя будущей карточки
    this._link = dataCard.link; // ссылка на картинку будущей карточки
...
>>>>>>> f7a2086a806c6e1aa36b6e62473435ecf142bf3f
    ...
    this._handleClickLikeCard = handleClickLikeCard; // поставить или убрать лайк карточке
    this._popupConfirmCardDelete = popupConfirmCardDelete; // мод. окно подтверждения согласия на удаление карточки
  }

  /* метод: получить копию разметки из темплейт для формирования карточки */
  _getTemplate() {
    return this._selectorTemplate.querySelector('.cards__item').cloneNode(true);
  }
  ...
  createCard() {
    ...
    this._selectorImage.src = this._link;
  ...
  _setCardTrash() {
    if (this._ownerId === this._personId) {
      ...
      this._selectorTrash.src = this._trash;
      this._selectorTrash.addEventListener('click', () =>
        this._popupConfirmCardDelete(this._cardId)
      ); // вешаем прослушиватель на удаления карточки
    } else {
  ...
  putCardLike(count) {
    this._selectorLike.classList.add('cards__like_active');
    this._selectorCountLike.textContent = `${count}`;
  }
  ...
  _setEventListeners() {
    this._selectorLike.addEventListener('click', () =>
      this._handleClickLikeCard(this._selectorLike, this._cardId)
    );
    this._selectorImage.addEventListener('click', () =>
      this._handleOpenImgCardClick(this._name, this._link)
    );
  }
